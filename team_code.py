...
from  image_to_signal import signal_restruct
...
#size: (num_signals,num_signals/4）:signal
    ...
    signal_250,signal_nan = signal_restruct(image_file_path , num_samples , num_signals)
    ...
    labels = (output > 0.5).float().detach().cpu().numpy()
    labels = np.squeeze(labels)
    print(labels)
    labels = [classes[i] for i, value in enumerate(labels) if value == 1]
    print(labels)
    return np.transpose(signal_nan), labels
